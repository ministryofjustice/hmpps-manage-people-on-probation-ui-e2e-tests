...
Then('I see {string} link on how to write questions page', async ({ctx}, linkName: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
    await managePage.assertCancelAndGoToOverviewLink(linkName)
})

When('I select {string} button on how to write questions page', async ({ctx}, buttonName: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
    await managePage.selectAddQuestionsToOnlineCheckInsButton()
})

Then('I see text {string} on add questions page', async ({ctx}, expectedText: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
    await managePage.assertTextOnAddQuestionPage(expectedText)
})

Then('I see default questions with preview links:', async ({ctx}, data: DataTable) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
    await managePage.assertQuestionsText(data)
    await managePage.assertPreviewLinks()
})

Then('I see {string} button on add questions page', async ({ctx}, expectedButton: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
...
Then('I see {string} link on add questions page', async ({ctx}, expectedLink: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
...
Then('I am directed to {string} page', async ({ctx}, expectedText: string) => {
    const managePage = new ManageCheckInsPage(ctx.base.page)
    await managePage.assertTextOnAddQuestionPage(expectedText)
    }
)
