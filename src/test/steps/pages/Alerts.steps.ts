...
When("I click the person link", async ({ ctx }) => {
  ...
  const row = alerts.getClass("govuk-table__row").filter({
    has: page.getByRole("cell", {
      name: `${person.lastName}, ${person.firstName} ${crn}`,
    }),
  });
  await alerts
    .getQA("alertPerson", row)
    .getByRole("link", { name: `${person!.lastName}, ${person.firstName}` })
    .first()
    .click();
});
...
When("I click the activity link", async ({ ctx }) => {
  ...
  await alerts
    .getQA("alertActivity", row)
    .getByRole("link", { name: "3 Way Meeting (Non NS)" })
    .first()
    .click();
});
...
When("I view the activity note", async ({ ctx }) => {
  ...
  await alerts
    .getQA("alertActivity", row)
    .getByRole("link", { name: "View full note" })
    .click();
});
