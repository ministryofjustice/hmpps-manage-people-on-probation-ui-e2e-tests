...
export default class HomePage extends MPopPage {
  ...
  async logMoreOutcomes() {
    await this.getQA("homepage-outcomes")
      .getByRole("link", { name: "Log more outcomes" })
      .click();
  }

  async viewUpcoming() {
    await this.getQA("homepage-appointments")
      .getByRole("link", { name: "View all upcoming appointments" })
      .click();
  }
  ...
  async noteUpcoming(): Promise<string[]> {
    ...
    const text = await this.getQA("homepage-appointments")
      .getByRole("link", { name: /,/ })
      .allTextContents();
  ...
  async noteOutcomes(): Promise<string[]> {
    ...
    const text = await this.getQA("homepage-outcomes")
      .getByRole("link", { name: /,/ })
      .allTextContents();
    return text.map((i) => i.trim());
  }
