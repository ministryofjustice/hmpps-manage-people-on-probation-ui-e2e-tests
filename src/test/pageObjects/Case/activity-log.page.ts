...
export default class ActivityLogPage extends CasePage {
  ...
  async applyFilters(filters: ContactFilters) {
    const complianceGroup = this.page.getByRole("group", {
      name: "Compliance filters",
    });
    const categoryGroup = this.page.getByRole("group", {
  ...
  async checkAvailable(): Promise<boolean> {
    try {
      ...
      for (const entry of await this.getClass(
        "govuk-details",
        this.getTimelineCard(1),
      )
        .filter({ hasText: "Online probation check in" })
        .all()) {
        try {
          await entry.click();
          await expect(
            this.getClass("govuk-details__text", entry),
          ).toContainText("Check in status: Missed");
          return false;
        } catch {
          continue;
        }
  ...
  getFilter(id: number, qa: string) {
    return this.getClass("govuk-checkboxes__item", this.getQA(qa))
      .nth(id)
      .getByRole("checkbox");
  }
