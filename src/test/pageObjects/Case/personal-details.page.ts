...
export default class PersonalDetailsPage extends CasePage {
  ...
  async checkForPractitioner(): Promise<boolean> {
    try {
      expect(
        await this.getSummaryRowByKey("Probation practitioner"),
      ).toBeDefined();
      await expect(
        await this.getSummaryRowValue(
          await this.getSummaryRowByKey("Probation practitioner"),
        ),
      ).not.toHaveText("Unallocated");
      return true;
    } catch {
      return false;
    }
  ...
  async assertSections(data: DataTable) {
    for (const row of data.rows()) {
      ...
      switch (label) {
        case "Phone number":
          await expect(this.getQA("telephoneNumberLabel")).toContainText(
            "Phone number",
          );
          break;
        case "Mobile number":
          await expect(this.getQA("mobileNumberLabel")).toContainText(
            "Mobile number",
          );
          break;
        case "Email Address":
          await expect(this.getQA("emailAddressLabel")).toContainText(
            "Email address",
          );
          break;
        case "Main Address":
          await expect(this.getQA("mainAddressLabel")).toContainText(
            "Main address",
          );
          break;
        case "Other addresses":
          await expect(this.getQA("otherAddressesLabel")).toContainText(
            "Other addresses",
          );
          break;
        case "Contacts":
        ...
        case "Name":
          ...
          break;
        case "Date of birth":
          await expect(this.getQA("dateOfBirthLabel")).toContainText(
            "Date of birth",
          );
          break;
        case "Preferred name/Known as":
          await expect(this.getQA("preferredNameLabel")).toContainText(
        ...
        case "Aliases":
          ...
          break;
        case "Previous name":
          await expect(this.getQA("previousSurnameLabel")).toContainText(
            "Previous name",
          );
          break;
        case "Preferred language":
          await expect(this.getQA("preferredLanguageLabel")).toContainText(
            "Preferred language",
          );
          break;
        case "Current circumstances":
          await expect(this.getQA("currentCircumstancesLabel")).toContainText(
            "Current circumstances",
          );
          break;
        case "Disabilities":
          await expect(this.getQA("disabilitiesLabel")).toContainText(
            "Disabilities",
          );
          break;
        case "Adjustments":
          await expect(this.getQA("adjustmentsLabel")).toContainText(
            "Adjustments",
          );
          break;
        case "Personal documents":
          await expect(this.getQA("documentsLabel")).toContainText(
            "Personal documents",
          );
          break;
        case "CRN":
        ...
        case "Probation practitioner":
          await expect(
            this.getQA("staffContactRoleLabel").first(),
          ).toContainText("Probation practitioner");
          break;
        case "Prison offender manager (POM)":
          await expect(
            this.getQA("staffContactRoleLabel").last(),
          ).toContainText("Prison offender manager (POM)");
          break;
        case "Religion or belief":
          await expect(this.getQA("religionOrBeliefLabel")).toContainText(
            "Religion or belief",
          );
          break;
        case "Sex":
        ...
        case "Gender identity":
          await expect(this.getQA("genderIdentityLabel")).toContainText(
            "Gender identity",
          );
          break;
        case "Self-described gender":
          await expect(this.getQA("selfDescribedGenderLabel")).toContainText(
            "Self-described gender",
          );
          break;
        case "Sexual orientation":
          await expect(this.getQA("sexualOrientationLabel")).toContainText(
            "Sexual orientation",
          );
          break;
      }
