...
export default class DateFrequencyPage extends ContactPage {
  ...
  async selectOption2Weeks() {
    await this.page.waitForURL(/\/check-in\/date-frequency/, {
      timeout: 20000,
    });
    ...
    const radioButton = this.page.locator(
      'input[type="radio"][value="TWO_WEEKS"]',
    );
  ...
  async selectOption4Weeks() {
    ...
    const radioButton = this.page.locator(
      'input[type="radio"][value="FOUR_WEEKS"]',
    );
    await radioButton.isEnabled();
    await radioButton.check();
  ...
  async updateToNextWeekDate() {
    ...
    if (value !== formattedDate) {
      throw new Error(
        `Date input failed. Expected: ${formattedDate}, got: ${value}`,
      );
    }
