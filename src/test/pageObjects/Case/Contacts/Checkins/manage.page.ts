...
export default class ManageCheckInsPage extends ContactPage {
        ...
        for (const row of data.rows()) {
            ...
            switch (expectedQuestionText) {
                case 'How have you been feeling since we last spoke?':
                    ...
                    await expect(this.page.locator(`[class=govuk-table__cell]`).first()).toContainText(expectedQuestionText)
                    break;
                case 'Is there anything you need support with or want to let us know about?1':
                    await expect(this.page.locator(`[class=govuk-table__cell]`).nth(2)).toContainText(expectedQuestionText)
                    break;
            }
    ...
    async assertSaveQuestionsButton(expectedButton: string){
        await expect(this.getQA(`save-questions-btn`)).toContainText(expectedButton)
    }
