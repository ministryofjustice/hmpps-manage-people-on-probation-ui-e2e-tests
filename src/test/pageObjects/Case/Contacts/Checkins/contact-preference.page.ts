...
export default class ContactPreferencePage extends ContactPage {
  ...
  async completePage(values: ContactDetails, preference: Preference) {
    await this.changePage(values, preference);
  }

  async changePage(values?: ContactDetails, preference?: Preference) {
  ...
  async enterContactPreferenceIfDoesNotExists(
    ...
    switch (contactMethod) {
      ...
      case "emailUpdate":
        ...
        inputLocator = this.page.locator(
          'input[aria-describedby$="editCheckInEmail-hint"]',
        );
        break;
