...
export default class RescheduleDetailsPage extends ContactPage {
  ...
  async checkSensitivity(
    sensitivity: string,
    sensitivityLocked: boolean = false,
  ) {
    await this.checkRow("Sensitivity", sensitivity, !sensitivityLocked);
  }
  ...
  async checkPageFuture(details: MpopArrangeAppointment) {
    ...
    await this.checkSensitivity(details.sensitivity, details.sensitivityLocked);
  }
}
