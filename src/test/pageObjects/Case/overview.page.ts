...
export default class OverviewPage extends CasePage {
  ...
  async checkOnlineCheckInsSetup(): Promise<boolean> {
    try {
      await expect(this.getQA("checkinCard").getByRole("link")).toHaveText(
        "View all online check in details",
        { timeout: 1000 },
      );
      return true;
    } catch {
  ...
  async checkAppointmentsLink() {
    await this.checkLink(
      "scheduleCard",
      "View appointments",
      new AppointmentsPage(this.page),
    );
  }
  ...
  async checkPersonalDetailsLink() {
    await this.checkLink(
      ...
      new PersonalDetailsPage(this.page),
    );
  }
  async checkRiskLink() {
    await this.checkLink(
      "riskCard",
      "View all risk details",
      new RiskPage(this.page),
    );
  }
  async checkSentenceLinks(ids: number[]) {
    for (let i = 0; i < ids.length; i++) {
      await this.checkLink(
        `sentence${ids[i]}Card`,
        "View all sentence details",
        new CaseSentencePage(this.page),
      );
    }
  }
