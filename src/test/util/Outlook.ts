...
export const checkOutlook = async(page: Page, caseInfo: CaseType, token: string, past: boolean, dateTime: MpopDateTime) => {
    ...
    if (past){
        ...
        expect(message).toContain(`${caseInfo.person.firstName[0]}. ${caseInfo.person.lastName}`)
    }
}